...
export function DeliveryNoteModal({ isOpen, onClose, onSuccess, deliveryNote }: DeliveryNoteModalProps) {
    ...
    if (field === 'order_id' && value) {
      ...
      if (selectedOrder) {
        newItems[index] = {
          ...
          delivery_address_street: extractStreetName(selectedOrder.customer.endereco) || '',
          delivery_address_number: extractStreetNumber(selectedOrder.customer.endereco) || '',
  ...
  // Helper functions to extract street name and number from address
  const extractStreetName = (address: string): string => {
    if (!address) return '';
    // Try to extract street name (everything before the last number)
  ...
  const extractStreetNumber = (address: string): string => {
    if (!address) return '';
    // Try to extract the last number in the address

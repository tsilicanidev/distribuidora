...
import { Plus, Search, FileText, AlertTriangle, CheckCircle, XCircle, Ban, Edit } from 'lucide-react';
...
function SalesOrders() {
  ...
  const handleApproveOrder = async (order: SalesOrder) => {
    ...
    try {
      ...
      // Generate a dummy XML for testing
      const xmlAssinado = "<NFe><infNFe>Dummy XML content for testing</infNFe></NFe>";

      // Emit NFe
      const nfeResult = await emitirNFe(xmlAssinado);
      if (!nfeResult.sucesso) {
        throw new Error(nfeResult.motivo || 'Erro ao emitir NFe');
      }
      ...
      const { error: invoiceError } = await supabase
        ...
        .insert([{
          ...
          status: 'issued',
          created_by: (await supabase.auth.getUser()).data.user?.id,
          xml_url: `/api/nfe/xml/${nfeResult.chave}`,
          pdf_url: `/api/nfe/danfe/${nfeResult.chave}`
        }]);

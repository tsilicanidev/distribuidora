/*
  ...
  1. New Tables
    - customer_order_links
      - Stores tokens for external customer order access
      - Links expire after 24 hours
      - Can be deactivated manually
    - customer_orders
      - Stores customer orders created through external links
      - Tracks order status (pending, approved, rejected)
  ...
  2. Security
    ...
    - Admin/Manager access policies for full control
    - Public read/write policies for active tokens
...
DROP POLICY IF EXISTS "admin_manager_access_orders" ON public.customer_orders;
DROP POLICY IF EXISTS "public_create_orders" ON public.customer_orders;
DROP POLICY IF EXISTS "public_read_orders" ON public.customer_orders;
DROP POLICY IF EXISTS "admin_manager_access_items" ON public.customer_order_items;
DROP POLICY IF EXISTS "public_create_items" ON public.customer_order_items;
DROP POLICY IF EXISTS "public_read_items" ON public.customer_order_items;

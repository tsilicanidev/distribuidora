...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    IF current_id = NEW.id AND NEW.role = profiles.role FROM profiles WHERE id = NEW.id THEN
      RETURN NEW;
    END IF;

    -- Only admins can change roles
    IF NEW.role != (SELECT role FROM profiles WHERE id = NEW.id) AND current_role NOT IN ('admin', 'master') THEN
...
DROP POLICY IF EXISTS "profiles_read_access_v5" ON profiles;
DROP POLICY IF EXISTS "profiles_self_update_v5" ON profiles;
DROP POLICY IF EXISTS "profiles_admin_master_access_v5" ON profiles;

-- Create new policies with unique names
CREATE POLICY "profiles_read_access_v6"
  ON profiles
  FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "profiles_self_update_v6"
  ON profiles
  FOR UPDATE
  TO authenticated
  USING (auth.uid() = id)
...
CREATE POLICY "profiles_admin_master_access_v6"
  ON profiles
  FOR ALL
  TO authenticated
  USING (

...
DO $$ 
BEGIN
  -- Drop all existing policies from vehicle_maintenance_records table
  DROP POLICY IF EXISTS "Vehicle maintenance records are viewable by authenticated users" ON vehicle_maintenance_records;
  DROP POLICY IF EXISTS "Vehicle maintenance records are modifiable by admins and managers" ON vehicle_maintenance_records;
  
  -- Drop all existing policies from driver_vehicles table
  DROP POLICY IF EXISTS "Driver vehicles are viewable by authenticated users" ON driver_vehicles;
  DROP POLICY IF EXISTS "Driver vehicles are modifiable by admins and managers" ON driver_vehicles;
END $$;

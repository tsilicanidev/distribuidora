/*
  # Delivery Notes and Fiscal Invoices Schema

  1. Tables Created
    - delivery_notes: Stores delivery note records
      - id, number, date, vehicle_id, route_id, status, notes, etc.
    - delivery_note_items: Links delivery notes to orders
      - id, delivery_note_id, order_id, delivery_sequence
    - fiscal_invoices: Stores fiscal invoice records
      - id, number, series, issue_date, customer_id, etc.

  2. Security
    - Enable RLS on all tables
    - Create unrestricted policies for authenticated users
    - Grant necessary permissions

  3. Performance
    - Add indexes for commonly queried columns
    - Create function for auto-generating delivery note numbers
*/

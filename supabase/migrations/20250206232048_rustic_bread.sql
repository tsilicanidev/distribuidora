...
-- Create driver_vehicles table for assignments if it doesn't exist
...
BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'driver_vehicles_view_policy'
  ) THEN
    CREATE POLICY "driver_vehicles_view_policy"
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'driver_vehicles' 
    AND policyname = 'driver_vehicles_modify_policy'
  ) THEN
    CREATE POLICY "driver_vehicles_modify_policy"

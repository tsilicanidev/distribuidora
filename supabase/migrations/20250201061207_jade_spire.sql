/*
  # Fix username_login function
  
  1. Drop existing function
  2. Recreate with unambiguous parameter names
*/
...
BEGIN
  ...
  -- Verify password
  IF auth.verify_password(input_password, _user.encrypted_password) THEN
    -- Create new session using auth.sign_in
    SELECT auth.sign_in('email', _user.email, input_password) INTO result;

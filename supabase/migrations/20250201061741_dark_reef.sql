/*
  # Update login system to use email
  
  1. Remove username-related items
  2. Update admin user with proper email format
*/
...
DO $$ 
BEGIN
  -- Set full_name to NOT NULL if it isn't already
  IF EXISTS (
    SELECT 1 
    FROM information_schema.columns 
    WHERE table_name = 'profiles' 
  ...
  -- Add full_name_format constraint if it doesn't exist
  IF NOT EXISTS (
    SELECT 1 
    FROM information_schema.table_constraints 
...
DECLARE
  admin_id uuid;
  admin_email text := 'tsilicani@gmail.com';  -- Changed to valid email format
BEGIN

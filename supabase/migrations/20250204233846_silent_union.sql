...
BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Stock movements view policy'
  ) THEN
    CREATE POLICY "Stock movements view policy"
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'stock_movements' 
    AND policyname = 'Stock movements modify policy'
  ) THEN
    CREATE POLICY "Stock movements modify policy"
